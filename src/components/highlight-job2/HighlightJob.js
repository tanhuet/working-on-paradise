...
const HighlightJob = (props) => {
  ...
  const companyNameAndLocation = scaleCompanyNameAndLocation(
    `${props.companyName}-${props.location}`
  );
  const experienceAndJobType = scaleExperienceAndJobType(
    `${props.experience},${props.jobType}`
  );
    ...
    if (userStore) {
      try {
        const resFlag = await axios.get(
          `${config.api.url}/job/${props.id}/marked`,
          { headers: { Authorization: `Bearer ${userStore.accessToken}` } }
        );
        if (!resFlag.data) {
          await axios.post(
            `${config.api.url}/job/${props.id}/mark`,
            {},
            { headers: { Authorization: `Bearer ${userStore.accessToken}` } }
          );
          navigate("/favourite");
